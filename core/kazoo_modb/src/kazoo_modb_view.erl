...
-spec get_results_descending(ne_binary(), ne_binary(), kz_json:path(), kz_json:path(), pos_integer(), mapper_fun(), kz_datamgr:view_options()) ->
                                    {api_integer(), kz_json:objects()}.
get_results_descending(AccountId, View, Start, End, Limit, Mapper, CouchOptions)
  when is_binary(AccountId), is_binary(View), is_integer(Start), is_integer(End), is_integer(Limit), Start > End ->
...
-spec get_results_ascending(ne_binary(), ne_binary(), kz_json:path(), kz_json:path(), pos_integer(), mapper_fun(), kz_datamgr:view_options()) ->
                                   {api_integer(), kz_json:objects()}.
get_results_ascending(AccountId, View, Start, End, Limit, Mapper, CouchOptions)
  when is_binary(AccountId), is_binary(View), is_integer(Limit) ->
...
get_results(?NE_BINARY = AccountId, ?NE_BINARY = View, Start, End, Options) ->
    ...
    case props:is_true(ascending, Options, false) of
        true ->
            get_results_ascending(AccountId, View, Start, End, Mapper, CouchOptions);
        false ->
            get_results_descending(AccountId, View, Start, End, Mapper, CouchOptions)
    end.

-spec get_results_descending(ne_binary(), ne_binary(), kz_json:path(), kz_json:path(), mapper_fun(), kz_datamgr:view_options()) -> kz_json:objects().
get_results_descending(AccountId, View, Start, End, Mapper, CouchOptions)
  when is_binary(AccountId), is_binary(View), is_integer(Start), is_integer(End), Start >= End ->
    MODbs = lists:reverse(kazoo_modb:get_range(AccountId, End, Start)),
    CouchOpts = [{startkey, Start}, {endkey, End}, descending | CouchOptions],
    [ apply_filter(unlimited_query(Db, View, CouchOpts), Mapper) || Db <- MODbs ].

-spec get_results_ascending(ne_binary(), ne_binary(), kz_json:path(), kz_json:path(), mapper_fun(), kz_datamgr:view_options()) -> kz_json:objects().
get_results_ascending(AccountId, View, Start, End, Mapper, CouchOptions)
  when is_binary(AccountId), is_binary(View), is_integer(Start), is_integer(End), Start < End ->
    MODbs = kazoo_modb:get_range(AccountId, Start, End),
    CouchOpts = [{startkey, Start}, {endkey, End} | CouchOptions ],
